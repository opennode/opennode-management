...
def ssh_connect_interactive_shell(user, host, port, transport, set_channel,
    ...
    protocol.ClientCreator(reactor, SSHClientTransport, transport, set_channel,
                           size, user, host, command).connectTCP(host, port)
...
class ClientUserAuth(userauth.SSHUserAuthClient):
    """Performs ssh connection authentication on behalf of a
    SSHClientTransport.

    Public key authentication is designed to be performed with the OMS service
    key.  The OMS thus takes the responsibility to authorize a given principal
    and then logs in on his behalf.

    """

    def __init__(self, *args, **kwargs):
        """Read the pub/priv keys from the standard location of the OMS home
        directory."""
        # Super is old-style class
        userauth.SSHUserAuthClient.__init__(self, *args, **kwargs)

        names = ['id_oms_dsa', 'id_oms_rsa', 'id_fake_dsa',
    ...
    def getPublicKey(self):
        """Return the public key unless we already failed our previous
        publickey auth attempt."""
    ...
    def getPassword(self, prompt=None):
        ...
        Writes a prompt and redirect the terminal input to a password reader
        which will callback a deferred and thus yield a password to conch.

        The terminal input will be automatically handled over to the ssh
...
class SSHShellConnection(connection.SSHConnection):
    """Represents a SSH client connection opening a interactive remote
    shell.
...
class ShellChannel(channel.SSHChannel):
    ...
    def channelOpen(self, data):
        ...
        data = session.packRequest_pty_req('xterm-color',
                                           (self.conn.terminal_size[1],
                                            self.conn.terminal_size[0], 0, 0),
                                           '')
